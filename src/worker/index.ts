...
app.get('/api/oauth/google/redirect_url', async (c) => {
  ...
  if (!apiUrl || !apiKey) {
    console.error('[Auth] Missing Mocha Users Service credentials:', {
      hasApiUrl: !!apiUrl,
      hasApiKey: !!apiKey,
    });
    return c.json({ 
      error: "Mocha Users Service not configured. Please set MOCHA_USERS_SERVICE_API_URL and MOCHA_USERS_SERVICE_API_KEY in .dev.vars file." 
    }, 500);
  }
  ...
  try {
    ...
    console.error('[Auth] Error getting OAuth redirect URL:', error);
    return c.json({ 
      error: error.message || "Failed to get OAuth redirect URL",
      details: "Check that MOCHA_USERS_SERVICE_API_URL and MOCHA_USERS_SERVICE_API_KEY are correctly set in .dev.vars"
    }, 500);
  }
});
